...
- 每个待克隆隔离对象显示为一个 foldout，顶部展示 `新建目标 / 覆盖已有目标 / TargetRoot 修复 / 显式共享 / 阻断 / 警告` 摘要。
...
- `写入清单` 分为 `新建目标资产`、`覆盖已有目标并保留 GUID`、`TargetRoot GUID 修复`、`显式共享，不写入`。
...
3. `AssetCloneIsolationOptions.ExplicitSharedAssetPaths` 保存用户主动标记为留在原地的 `SourceRoot` 依赖。
4. `AssetCloneIsolationService.BuildPlan` 只读生成计划。
5. 计划同时输出平铺写入清单和按 root 分组的关系视图数据。
6. 计划无错误时，报告中添加唯一写入操作 `应用资产克隆隔离计划`。
7. 用户点击 `应用计划` 后，TA Art Tools 调用模块 `Apply`，最终执行 `AssetCloneIsolationService.ApplyPlan`。
8. 用户点击 `审计 TargetRoot` 后，模块调用 `AuditTargetRoot(targetRoot, sourceRoot, explicitSharedAssetPaths)` 并展示只读审计报告。

## 核心接口

- `AssetCloneIsolationOptions`：保存源根、目标根、选中路径、显式共享路径、覆盖目标内容开关和目标目录修复开关。
- `AssetCloneIsolationPlan`：保存克隆项、GUID 映射、共享依赖、外部依赖、目标目录修复项、按 root 分组关系、错误、警告和信息。
- `AssetCloneIsolationRootPlan`：保存一个待克隆隔离对象的下游依赖、直接上游引用、共享依赖引用、相关写入项和风险汇总。
- `AssetCloneIsolationPlanSummary`：保存完整计划的新建、覆盖、修复、显式共享、阻断和警告统计。
...
- `AssetCloneIsolationPreset`：保存常用 `SourceRoot -> TargetRoot` 配置和显式共享路径。
...
- SourceRoot 外的非共享美术依赖只报错，不自动搬运。
- 二进制资源不做内部 GUID 重写。
- `TargetRoot` 已有文本资源只按本次 GUID 映射修复，不做其它重写。
- 显式共享只允许作用于依赖牵出的 `SourceRoot` 资源，直接选择的 root 资源始终克隆。
